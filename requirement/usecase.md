...
- 登録番号（要確認）
- 電話番号
- ログインID（任意のログインIDをTSPが決める。ログインIDは重複しない）
- メールアドレス（ログインIDにはしない）
...
- 氏名
- ログインID（任意のログインIDを指定する。システム全体でログインIDの重複は許さない）
- メールアドレス（オプション）
...
（現Chipsでは、休みを最大5日間に設定されているが、そのようなシフト入力時の制限があったほうがよいのか？要検討）
実際の運用では、多めに出勤日を登録して、その後の調整で休みが増えることもある。シフト希望の入力時に余裕のある
スケジュールを入力してもらう運用。TSPごとにこの最大日数を設定できる機能を用意する）

TODO: 最大休暇日数の設定機能のユースケースを追加

シフト決定までのフロー
1. クルーが出勤できる日、休みたい日をカレンダーに入力
2. TSMが翌月のたたき台としてシフトを作成
3. TSMはクルーが作成されたシフトを確認し問題なければ確定
4. 
...
クルーがシステムにログインし、個建配達実績入力ボタンをクリックする。
TODO: そもそもクルーが個建配達実績値を入力できるようにしてもよいのか？

どうやってクルーがTSMに個建の個数を伝えているか確認する。
→ クルーからTSMへの連絡はない？？
→ 布施さんに確認してもらう
...
メール等でURLを共有したい場合は、「PDFの共有」ボタンをクリックするとURLが生成され表示される。
...
TSMがシステムにログインし、出勤表をクリックすると当月の各クルーのシスト情報が表示される。
必要に応じて各クルーの出勤・休みのステータスを変更し、更新ボタンをクリックする。
（変更対象となったクルーには何らかの方法で通知が必要か？要検討。Astyさんはプリントアウトして確定シフトを共有することで運用している？？）
